...
Connection.prototype.fetchAll = function (statement, transaction, callback) {
	var self = this, data = [];
    const isPromise = statement.custom && !!statement.custom.on && typeof statement.custom.on.then === 'function';
    const isAsync = statement.custom && statement.custom.on && statement.custom.on.constructor.name === "AsyncFunction";
	var loop = async function (err, ret) {
		if (err) {
			return callback(err);
			...
			for (var i = 0; i < ret.data.length; i++) {
				...
				if (statement.custom && statement.custom.asStream && statement.custom.on) {
                    if (isAsync || isPromise) {
                        await statement.custom.on(ret.data[i], pos - 1);
                    } else {
